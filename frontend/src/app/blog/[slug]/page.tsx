...
function ShareButtons({ title, slug }: { title: string; slug: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="text-[9px] font-mono uppercase tracking-widest mr-1" style={{ color: 'var(--text-tertiary)' }}>Share</span>
      <a
      ...
      <button
        onClick={copyLink}
        className="w-8 h-8 rounded-lg flex items-center justify-center transition-all hover:text-accent"
        style={{ background: 'var(--accent-subtle)', color: copied ? 'var(--success)' : 'var(--text-secondary)', border: '1px solid var(--border)' }}
      >
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <BookOpen className="w-16 h-16 mx-auto text-accent/20 mb-4" />
    ...
    <div className="min-h-screen">
      ...
      <article className="max-w-4xl mx-auto px-4 py-10">
        ...
        <div className="flex items-center gap-2 mb-8 text-[10px] font-mono" style={{ color: 'var(--text-tertiary)' }}>
          ...
          <Link href={`/blog?category=${post.category}`} className="hover:text-accent transition-colors capitalize">
            {post.category}
          </Link>
          <ChevronRight className="w-3 h-3" />
        ...
        <header className="mb-8">
          ...
          <div className="flex items-center gap-3 mb-4">
            <span className="text-[9px] font-mono font-bold uppercase tracking-widest text-accent px-2.5 py-1 rounded"
            ...
            {post.isFeatured && (
              <span className="text-[9px] font-mono font-bold uppercase tracking-wider text-warning px-2 py-0.5 rounded"
                style={{ background: 'var(--warning-bg)', border: '1px solid rgba(255,170,0,0.2)' }}>
                Featured
              </span>
            )}
          </div>
          ...
          {/* Title */}
          <h1 className="text-3xl md:text-4xl lg:text-[2.75rem] font-semibold font-bold mb-6 leading-tight tracking-tight"
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6"
            ...
            <div className="flex items-center gap-3">
              {post.author?.avatar ? (
                <img src={post.author.avatar} alt="" className="w-11 h-11 rounded-full object-cover ring-2 ring-accent/15" />
              ) : (
                <div className="w-11 h-11 rounded-full flex items-center justify-center text-sm font-bold text-accent"
              ...
              <div>
                <Link href={`/profile/${post.author?.username}`}
                  className="text-sm font-semibold hover:text-accent transition-colors" style={{ color: 'var(--foreground)' }}>
        ...
        {/* Cover image */}
        {post.coverImage?.url && (
          <div className="mb-10 rounded-xl overflow-hidden" style={{ border: '1px solid var(--border)', boxShadow: 'var(--shadow-md)' }}>
        ...
        <div
          className="prose prose-invert dark:prose-invert max-w-none mb-10
            ...
            prose-blockquote:border-accent/30 prose-blockquote:bg-accent/5 prose-blockquote:rounded-r-lg prose-blockquote:py-1
        ...
        {post.tags && post.tags.length > 0 && (
          <div className="flex items-center gap-2 flex-wrap mb-8 pb-8" style={{ borderBottom: '1px solid var(--border)' }}>
            <Tag className="w-3.5 h-3.5 text-accent/40 shrink-0" />
            {post.tags.map((tag) => (
              <Link key={tag} href={`/blog?search=${tag}`}
                className="text-[10px] font-mono px-2.5 py-1 rounded transition-all hover:text-accent hover:border-accent/20"
        ...
        <div className="flex items-center justify-between mb-12">
          ...
          <Link href="/blog" className="text-[10px] font-mono flex items-center gap-1 hover:text-accent transition-colors"
            style={{ color: 'var(--text-secondary)' }}>
        ...
        {/* Author Card */}
        <div className="rounded-xl p-6 mb-12" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
          <div className="flex items-start gap-4">
            {post.author?.avatar ? (
              <img src={post.author.avatar} alt="" className="w-14 h-14 rounded-full object-cover ring-2 ring-accent/15 shrink-0" />
            ) : (
              <div className="w-14 h-14 rounded-full flex items-center justify-center text-lg font-bold text-accent shrink-0"
                style={{ background: 'var(--accent-muted)' }}>
                {post.author?.displayName?.[0] || '?'}
              </div>
            )}
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                ...
                {post.author?.isVerified && (
                  <span className="text-[8px] font-mono text-accent px-1.5 py-0.5 rounded"
                    style={{ background: 'var(--accent-muted)', border: '1px solid var(--border-strong)' }}>
                    Verified
                  </span>
                )}
              </div>
              <p className="text-[10px] font-mono mb-2" style={{ color: 'var(--text-tertiary)' }}>
      ...
      {relatedPosts.length > 0 && (
        <section className="max-w-4xl mx-auto px-4 pb-16">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h2 className="text-sm font-mono font-bold uppercase tracking-widest text-accent/70">
              Related Articles
            </h2>
          </div>
                  ...
                  <div className="aspect-video overflow-hidden">
                    {rp.coverImage?.url ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center" style={{ background: 'var(--surface)' }}>
                        <BookOpen className="w-8 h-8 text-accent/15" />
                      </div>
                    )}
                  </div>
                  <div className="p-3">
                    <span className="text-[8px] font-mono font-bold uppercase tracking-wider text-accent/60">{rp.category}</span>
                    <h3 className="text-xs font-semibold line-clamp-2 mt-1 group-hover:text-accent transition-colors"
                      style={{ color: 'var(--foreground)' }}>
                      {rp.title}
                    </h3>
                    <p className="text-[9px] font-mono mt-1.5 flex items-center gap-2" style={{ color: 'var(--text-tertiary)' }}>
