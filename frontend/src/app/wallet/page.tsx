  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-10">
      <h1 className="text-2xl font-display font-bold mb-8 flex items-center gap-3" style={{ color: 'var(--edith-text)' }}>
        <Wallet className="w-6 h-6 text-edith-cyan" /> Credits &amp; Wallet
      </h1>
      ...
      <div className="card p-6 mb-6 relative overflow-hidden">
        ...
        <p className="text-4xl font-display font-bold text-edith-cyan">
          ${wallet?.balance?.toFixed(2) || '0.00'}
        </p>
        <p className="text-[10px] font-mono text-[var(--edith-text-dim)] mt-1">
          {wallet?.currency || 'USD'}
        </p>
      ...
      {showTopUp && (
        <div className="card p-6 mb-6">
          ...
          <div className="flex gap-2 mb-4">
            <button onClick={() => setTopUpCurrency('USD')} className={`px-3 py-1.5 rounded text-xs font-mono flex items-center gap-1 ${topUpCurrency === 'USD' ? 'bg-edith-cyan/20 text-edith-cyan border border-edith-cyan/30' : 'bg-[var(--edith-surface)] text-[var(--edith-text-dim)] border border-[var(--edith-border)]'}`}>
              <DollarSign className="w-3 h-3" /> USD
            </button>
            <button onClick={() => setTopUpCurrency('INR')} className={`px-3 py-1.5 rounded text-xs font-mono flex items-center gap-1 ${topUpCurrency === 'INR' ? 'bg-edith-cyan/20 text-edith-cyan border border-edith-cyan/30' : 'bg-[var(--edith-surface)] text-[var(--edith-text-dim)] border border-[var(--edith-border)]'}`}>
              <IndianRupee className="w-3 h-3" /> INR
            </button>
          </div>
          <div className="flex flex-wrap gap-2 mb-4">
            {presets.map((p) => (
              <button key={p} onClick={() => setTopUpAmount(p)} className={`px-3 py-1.5 rounded text-xs font-mono ${topUpAmount === p ? 'bg-edith-cyan/20 text-edith-cyan border border-edith-cyan/30' : 'bg-[var(--edith-surface)] text-[var(--edith-text-dim)] border border-[var(--edith-border)]'}`}>
                {topUpCurrency === 'USD' ? '$' : '₹'}{p}
              </button>
            ))}
          </div>
          ...
          <p className="text-[10px] font-mono text-[var(--edith-text-dim)] mt-2">
      ...
      {/* Transaction History */}
      <h3 className="text-xs font-mono font-bold uppercase tracking-wider text-edith-cyan/60 mb-3">Transaction History</h3>
          ...
          {wallet.transactions.map((tx, i) => (
            <div key={i} className="card p-4 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs font-mono" style={{ color: 'var(--edith-text)' }}>{tx.description || (tx.type === 'credit' ? 'Wallet Top-up' : 'Payment')}</p>
